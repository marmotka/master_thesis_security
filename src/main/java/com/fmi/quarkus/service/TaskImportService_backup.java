...
@ApplicationScoped
public class TaskImportService_backup {
    ...
    public ImportResult importFromCsv(File file, String username) throws IOException {
        ...
        String originalFilename = getOriginalFilenameFromUpload(file); // see below
    ...
    private String getOriginalFilenameFromUpload(File tempFile) throws IOException {
        // Quarkus stores the original filename in a sibling .upload file
        Path metaFile = Path.of(tempFile.getAbsolutePath() + ".upload");
        if (Files.exists(metaFile)) {
            String metaContent = Files.readString(metaFile);
            // Format: filename="user-uploaded-name.csv"; size=1234
            return metaContent.lines()
                    .filter(line -> line.startsWith("filename=\""))
                    .findFirst()
                    .map(line -> line.substring(10, line.length() - 1)) // remove filename=" and "
                    .orElse(tempFile.getName());
        }
